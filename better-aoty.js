...
document.querySelectorAll(".ratingText").forEach((text) => {
  if (text.textContent.trim() === "user score") {
    text.remove();
  }
});
...
document.querySelectorAll(".rightBox").forEach((box) => {
  if (box.textContent.includes("LINKS")) {
    box.remove();
  }
});

document.querySelectorAll(".section").forEach((section) => {
  if (section.textContent.includes("You May Also Like")) {
    section.remove();
  }
});
...
document.querySelectorAll("a").forEach((link) => {
  if (link.textContent.includes("Critic")) {
    link.remove();
  }
});
...
document.querySelectorAll(".buyButtons").forEach((el) => el.remove());
